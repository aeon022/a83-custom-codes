...
(function(){
  ...
  (function initHeaderMinimalAntiJump(){
    ...
    headerEl.addEventListener('click', (e)=>{
      ...
      if(!a || !headerEl.contains(a)) return;
      const href = a.getAttribute('href') || '';
      if(isPlaceholderHref(href)){
        e.preventDefault(); // do not stop propagation; allow other handlers
      }
    }, true);
  ...
  function lockBody(){
    ...
    setScrollRestoration(true);            // prevent native jumps while locked
    ...
    s.width = '100%';
    s.overflow = 'hidden';
    document.body.setAttribute('data-a83-locktop', s.top);
  }

  function unlockBody(){
    const topStr = document.body.getAttribute('data-a83-locktop') || document.body.style.top || '0';
    const y = Math.max(0, Math.round(-parseFloat(topStr) || scrollY || 0));
    ...
    s.width = '';
    s.overflow = '';
    document.body.removeAttribute('data-a83-locktop');
