/* Off-Canvas A83 — v13.5
   (WILD + Hint single-line hard + Smooth Close mit Blur-Delay)
   - Custom Burger (#nav-trigger.nav-toggle) mit Open/Reset-Animation
   - ESC/Close: sauberer Reset (aria-expanded=false, .is-open weg) + kurzer Recoil
...
(function(){
  ...
  // Burger-Anim Zeiten (nur Klassen-Toggles; Optik via CSS)
  ...
  // Optionale Dancepad-Unterstützung (falls intern noch ein DP-Button existiert)
  const dpButton = () =>
    burger.querySelector('button, [role="button"], .dan-burger, .hamburger, .dan-burger-animated--distorsionv3');

  function resetDpVisualIfAny(){
    const inner = dpButton();
    if(inner){
      try { ignoreNext = true; inner.click(); } catch(e){}
    }
  }

  // Burger-Animationen (per CSS-Klassen)
  ...
  function closeNav(){
    ...
    // Kurzer visueller Puls vorm Out
    if(!prefersRM()){
      pulse(card, 'glitch-strong', 220);
      pulse(menu, 'glitch-once',   200);
    }

    // Burger: Recoil-Animation starten
    playBurgerReset();
    // aria/State früh zurück (dezent verzögert, damit Recoil sichtbar bleibt)
    setTimeout(()=> setBurgerOpen(false), Math.min(120, BURGER_RESET_DUR * 0.35));

    // DP (falls vorhanden) visuell neutralisieren
    resetDpVisualIfAny();

    // In "closing" wechseln; Card-Out + Blur-Out laufen via CSS
    setTimeout(()=>{
      setState('closing');
      ...
      setTimeout(()=>{
        ...
        // Absicherung: Burger-State sauber
        setBurgerOpen(false);
  ...
  console.info('[nav] init ok (v13.5)');
})();