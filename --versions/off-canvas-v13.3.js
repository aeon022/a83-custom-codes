/* Off-Canvas A83 — v13.3 (WILD + Hint single-line hard + delayed Close-Glitch)
   ...
   - Close-Glitch: kurzer Puls mit Verzögerung vor dem eigentlichen Fade-Out
...
(function(){
  ...
  const T_CARD_OUT     = cssMs('--t-card-out',     360);
  const T_BACKDROP_OUT = cssMs('--t-backdrop-out', 260);
  const G_CARD         = cssMs('--glitch-card-dur',640);
  const G_LINK         = cssMs('--glitch-link-dur',520);

  // Close-Glitch Verzögerung (sichtbar bevor Out-Transition startet)
  ...
  // ===== Terminal-Hint (IMMER SINGLE-LINE + Grid-Absicherung)
  const CMD_PREFIX = '$ ';
  const CMD_VARIANTS = [
    t => `open /${t}`,
    t => `lynx /${t}`,
  ...
  function enforceSingleLineHint(){
    // absolute Absicherung auch per Inline-Styles (+ !important via setProperty)
  ...
  function resetDpVisual(){
    ...
    if(inner){ ignoreNext = true; inner.click(); }
    setExpanded(false);
    dpResetDone = true;
  }
  ...
  function openNav(){
    ...
    closeBtn.classList.add('x-hidden');   // Close-X später einblenden
  ...
  // ===== Schließen (mit sichtbarer Verzögerung für Glitch)
  function closeNav(){
    if(state!=='open' && state!=='opening') return;

    // 1) Kurz Glitch sichtbar lassen, während der State noch "open" ist
    if(!prefersRM()){
      pulse(card, 'glitch-strong', 220);
      pulse(menu, 'glitch-once',   200);
    }

    // 2) Nach kurzer Verzögerung erst in den "closing" State wechseln (dadurch sieht man den Glitch)
    setTimeout(()=>{
      setState('closing');
      resetDpVisual();

      const total = T_CARD_OUT + T_BACKDROP_OUT + 20;
      setTimeout(()=>{
  ...
  console.info('[nav] init ok (v13.3)');
})();
