/* ============================================================
   A83 HERO – Hover→Video + DOUBLE GLITCH (self-contained)
...
(function(){
  ...
  const text   = hero?.querySelector('#text, .text');
  const stage  = hero?.querySelector('#show-max');
  const wireA  = text?.querySelector('.neon-wire');
  const wireB  = stage?.querySelector('.neon-wire-under');
  if(!hero || !text || !stage || !wireA) return;

  function num(val, def=0){ const n = parseFloat(val); return isFinite(n) ? n : def; }
  function rad(d){ return Math.abs(d) * Math.PI / 180; }

  function apply(){
    const tr = text.getBoundingClientRect();
    const sr = stage.getBoundingClientRect();

    // echte horizontale Distanz Text → Stage (Gap)
    const gap = Math.max(0, sr.left - tr.right);

    // Winkel-Korrektur: projizierte Sichtlänge vs. Elementbreite
    const cs  = getComputedStyle(hero);
    const ang = num(cs.getPropertyValue('--wire-angle'), -10);
    const c   = Math.cos(rad(ang)) || 1;

    // Segment A: genau bis zur Stagekante (kein Overlap, damit Ende unter Stage verschwindet)
    const segA = gap / c;
    wireA.style.width = segA + 'px';

    // Segment B: „hinter“ das Visual – nimmt deine CSS-Var oder fallback aus Stagebreite
    let into = cs.getPropertyValue('--wire-into-stage').trim();
    let intoPx = parseFloat(into);
    if(!isFinite(intoPx)){
      // Fallback, wenn clamp/vw o.ä.: nimm % von Stagebreite (wirksam & ruhig)
      intoPx = Math.max(0.4 * sr.width, 240);
    }
    const segB = intoPx / c;
    if(wireB){
      wireB.style.width = segB + 'px';
    }
  }

  // initial + bei Resize (debounced)
  let t;
  apply();
